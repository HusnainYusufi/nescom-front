...
import {
  CContainer, CRow, CCol, CButton, CAlert, CTable, CTableHead, CTableRow, CTableHeaderCell,
  CTableBody, CTableDataCell, CModal, CModalHeader, CModalTitle, CModalBody, CModalFooter,
  CForm, CFormLabel, CFormInput, CBadge, CInputGroup, CInputGroupText, CFormSelect, CSpinner,
  CPagination, CPaginationItem
} from '@coreui/react'
import CIcon from '@coreui/icons-react'
import {
  cilPlus, cilSearch, cilCloudDownload, cilSync, cilTrash, cilSortAscending, cilSortDescending,
} from '@coreui/icons'
...
const Warehouses = () => {
  ...
  const sorted = useMemo(() => {
    ...
    const getter = (w) => {
      switch (sortBy) {
        case 'name': return String(w.name || '')
        case 'city': return String(w?.address?.city || '')
        case 'country': return String(w?.address?.country || '')
        case 'person': return String(w?.contact?.person || '')
        default: return String(w.name || '')
      }
    }
    arr.sort((a, b) => {
  ...
  const exportCsv = () => {
    const headers = [
      'Name', 'Address Line 1', 'City', 'Country', 'Contact Person', 'Phone', 'Email'
    ]
    ...
    <CContainer className="mt-4 warehouses-page">
      ...
      <div className="wp-toolbar" ref={tableTopRef}>
        <div className="left">
          ...
          <div className="sub">
            <CBadge color="dark" className="me-2">{warehouses.length} total</CBadge>
            ...
            <CTableHead className="sticky-head">
              <CTableRow>
                <CTableHeaderCell onClick={() => toggleSort('name')} role="button">
                  Name {sortBy === 'name' ? <CIcon icon={sortDir === 'asc' ? cilSortAscending : cilSortDescending} /> : null}
                </CTableHeaderCell>
                <CTableHeaderCell>Address</CTableHeaderCell>
                <CTableHeaderCell onClick={() => toggleSort('city')} role="button" className="th-tight">
                  City {sortBy === 'city' ? <CIcon icon={sortDir === 'asc' ? cilSortAscending : cilSortDescending} /> : null}
                </CTableHeaderCell>
                <CTableHeaderCell onClick={() => toggleSort('country')} role="button" className="th-tight">
                  Country {sortBy === 'country' ? <CIcon icon={sortDir === 'asc' ? cilSortAscending : cilSortDescending} /> : null}
                </CTableHeaderCell>
                <CTableHeaderCell onClick={() => toggleSort('person')} role="button">
                  Contact Person {sortBy === 'person' ? <CIcon icon={sortDir === 'asc' ? cilSortAscending : cilSortDescending} /> : null}
                </CTableHeaderCell>
              ...
              {loading && Array.from({ length: 6 }).map((_, i) => (
                <CTableRow key={`sk-${i}`} className="skeleton-row">
                  {Array.from({ length: 8 }).map((__, j) => (
                    <CTableDataCell key={`skc-${i}-${j}`}><div className="sk"/></CTableDataCell>
                  ))}
                </CTableRow>
              ))}
              ...
              {!loading && paginated.map((w) => {
                ...
                return (
                  <CTableRow key={id} className="wp-row">
                    ...
                    <CTableDataCell className="nowrap">{w?.address?.country || '—'}</CTableDataCell>
                    <CTableDataCell>
                    ...
                    <CTableDataCell className="nowrap">
                      {w?.contact?.email ? <a href={`mailto:${w.contact.email}`}>{w.contact.email}</a> : '—'}
                    </CTableDataCell>

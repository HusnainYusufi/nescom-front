...
const ProductionHome = () => {
  ...
  const buildButton = (action) => (
    <Link
      key={action.label}
      to={action.to}
      className="btn btn-outline-primary bg-white text-start w-100 p-4 shadow-sm border-2 rounded-4 d-flex justify-content-between align-items-center h-100"
    >
      <div className="pe-3">
        <div className="d-flex align-items-center gap-2 mb-2">
          <CIcon icon={cilLibraryAdd} className="text-primary" size="lg" />
          <span className="fw-bold fs-5 text-primary">{action.label}</span>
        </div>
        <small className="text-body-secondary lh-base">{action.description}</small>
      </div>

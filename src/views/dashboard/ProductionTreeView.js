...
const ProductionTreeView = () => {
  ...
  return (
    <CContainer fluid className="py-4">
      <CRow className="g-4 mb-1">
        <CCol>
          <CCard className="shadow-sm border-0">
            <CCardHeader className="bg-primary bg-opacity-10 fw-semibold">General Main Form now includes:</CCardHeader>
            <CCardBody>
              <CListGroup flush className="mb-0">
                {generalSections.map((item) => (
                  <CListGroupItem key={item.title} className="py-3 px-2 border-0 border-bottom">
                    <div className="fw-bold text-decoration-underline text-danger">{item.title}</div>
      ...
      <CRow className="g-4">
        <CCol lg={5} xl={4}>
          <CAlert color="secondary" className="h-100 mb-0">
